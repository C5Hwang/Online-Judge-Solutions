...
int n , m , a[MAX_N] , b[MAX_N] , c[MAX_M] , f[MAX_M] , g[MAX_M] , rev[MAX_M] ;
...
void work(int n) {
	...
	init(n << 1) ;
	for (int i = 0 ; i <= n ; ++i) f[i] = b[i] , g[i] = a[i] ;

	fnt(f , 1) ; fnt(g , 1) ; fnt(c , 1) ;
	for (int i = 0 ; i < m ; ++i) f[i] = ((ll)f[i] * f[i] + g[i]) % mod * ny % mod * c[i] % mod ;
	fnt(f , -1) ;

	ny = qpow(m , mod - 2) ;
	for (int i = 0 ; i <= n ; ++i) b[i] = (ll)ny * f[i] % mod ;
}
