...
ldb f[MAX_N] ;
char s[MAX_N][MAX_L] ;
int T , n , L , P , tot , le , a[MAX_N] , S[MAX_N] , p[MAX_N] ;
...
int main() {
	...
	for (; T-- ;) {
		...
		for (int i = 2 ; i <= n ; ++i) {
			...
			f[i] = getf(i , sta[le].pt) ; p[i] = sta[le].pt ;
		}
		...
		if (f[n] - INF < eps) {
			...
			for (int i = n ; i ; i = p[i] - 1) p[p[i]] = i ;
			for (int i = 1 ; i <= n ; i = p[i] + 1)
				for (int k = i ; k <= p[i] ; ++k) {
					for (int l = 0 ; l < a[k] ; ++l) printf("%c" , s[k][l]) ;
					if (k < p[i]) printf(" ") ; else printf("\n") ;
				}
		}
