...
void build(int m) {
	...
	for (int i = n - 1 ; i >= 0 ; --i) sa[--cnt[s1[i]]] = i ;
...
int main() {
	...
	scanf("%d" , &n) ; len = n ;
	scanf("%s %s" , s1 , s2) ;
	for (int i = 0 ; i < len ; ++i) bh[i] = 0 , maxa = std::max(maxa , (int)s1[i]) ;
	bh[n] = -1 ; s1[n++] = '#' ;
	for (int i = 0 ; i < len ; ++i) bh[n] = 1 , maxa = std::max(maxa , (int)s2[i]) , s1[n++] = s2[i] ;
	s1[n++] = 0 ;
	...
	for (int i = 1 ; i < n ; ++i) {
		...
		if (height[i] > res) res = height[i] , p = sa[i] ;
	}

	for (int i = p ; i < p + res ; ++i)	printf("%c" , s1[i]) ;
	printf("\n") ;
